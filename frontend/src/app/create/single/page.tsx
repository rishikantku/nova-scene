...
export default function Home() {
  ...
  const handleSaveScene = async (sceneId: string) => {
    ...
    try {
      ...
      addLog(`Scene updated via Director's Cut.`);
      setEditingSceneId(null);
    } catch (err: any) {
      addLog(`Error saving scene: ${err.message}`);
    }
  };
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      const payload: any = { prompt, aspect_ratio: aspectRatio, duration_target: duration, video_engine: videoEngine, visual_style: visualStyle };
      if (includeAudio) {
        ...
        <section className="lg:col-span-5 flex flex-col gap-6">
          <div className="bg-[#0b0a0f]/80 border border-white/10 rounded-2xl p-6 backdrop-blur-lg flex flex-col gap-5 shadow-2xl relative">
            ...
            <form onSubmit={handleGenerate} className="flex flex-col gap-4">
              ...
              {/* Row 2: Configuration */}
            <div className="bg-black/40 border border-white/5 rounded-xl p-5 mb-8 backdrop-blur-xl">
                    ...
                    <div className="flex-1 flex flex-col p-6 overflow-y-auto max-h-[500px]">
                      ...
                      <div className="flex flex-col gap-4 mb-6">
                        {job.scenes.map((scene) => (
                          <div key={scene.id} className="bg-[#121118]/80 border border-white/10 rounded-xl p-4 shadow-lg border-l-2 border-l-amber-500/50 flex flex-col gap-3">
                            {editingSceneId === scene.id ? (
                              <>
                                <div className="flex justify-between items-center">
                                  <span className="text-xs font-bold text-violet-400 tracking-wider">EDITING SCENE {scene.index + 1}</span>
                                  <div className="flex items-center gap-2">
                                    <input 
                                      type="number" 
                                      min={1} 
                                      ...
                                      className="w-16 bg-black/40 border border-white/10 rounded px-2 py-1 text-xs font-mono text-[#f5f5f7] focus:outline-none focus:border-violet-500"
                                    />
                                ...
                                <textarea
                                  ...
                                  rows={3}
                                  className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-violet-500/50 transition-colors text-zinc-300 font-mono resize-none"
                                />
                                <div className="flex justify-end gap-2 mt-1">
                                  <button onClick={handleCancelEdit} className="p-1.5 hover:bg-white/5 rounded text-zinc-400 transition-colors" title="Cancel">
                                    <X className="w-4 h-4" />
                                  </button>
                                  <button onClick={() => handleSaveScene(scene.id)} className="p-1.5 hover:bg-emerald-500/20 text-emerald-400 rounded transition-colors" title="Save Scene">
                              ...
                              <>
                                <div className="flex justify-between items-center">
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={() => handleEditScene(scene)} className="text-zinc-500 hover:text-violet-400 transition-colors" title="Edit Scene">
